...
class MerkleTree:
    ...
    def __init__(self, items):
        ...
        self.leaves = map(self._leafify, map(self.sha256Sum, items))
    ...
    def _audit(self, questioned_hash, proof_hashes):
        ...
        if child.mom.hash == questioned_hash:
            actual_hash = self.sha256Sum(questioned_hash + sibling.hash)
        elif child.dad.hash == questioned_hash:
            actual_hash = self.sha256Sum(sibling.hash + questioned_hash)
        else:
            return False
